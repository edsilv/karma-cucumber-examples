addStepDefinitions(function (scenario) {
    ...
    scenario.Given(/^the box is visible$/, function (callback) {
        $box = $('.box');

        if ($box.is(':visible')){
            callback();
        } else {
            callback.fail("The box was not visible");
        }
    });

    scenario.Then(/^the box contains the text 'The box'$/, function (callback) {
        if ($box.text() == "The box"){
            callback();
        } else {
            callback.fail("The box didn't contain the text 'The box'");
        }
    });
